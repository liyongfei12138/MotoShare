...
//布局类
class CardSwitchFlowLayout: UICollectionViewFlowLayout {
    //卡片和父视图宽度比例
    let cardWidthScale: CGFloat = 0.6
    //卡片和父视图高度比例
    let cardHeightScale: CGFloat = 0.9
    //滚动到中间的调方法
    var indexChangeBlock: XLCardScollIndexChangeBlock?
    
    override func prepare() {
        self.scrollDirection = UICollectionView.ScrollDirection.horizontal
        self.sectionInset = UIEdgeInsets(top: self.insetY(), left: self.insetX() - 50, bottom: self.insetY(), right: self.insetX())
...
//代理
@objc protocol CardSwitchDelegate: NSObjectProtocol {
    //滑动切换到新的位置回调
...
//数据源
@objc protocol CardSwitchDataSource: NSObjectProtocol {
    //卡片的个数
...
//展示类
class HomeCardSwitch: UIView ,UICollectionViewDelegate,UICollectionViewDataSource {
    //公有属性
    weak var delegate: CardSwitchDelegate?
    weak var dataSource: CardSwitchDataSource?
    ...
    private let flowlayout = CardSwitchFlowLayout()
